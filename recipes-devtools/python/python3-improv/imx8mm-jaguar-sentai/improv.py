...
class ImprovProtocol:
    ...
    def parse_improv_data(self, data: bytearray) -> tuple:
        ...
        if (command == ImprovCommand.WIFI_SETTINGS):
            ...
            if password_end < len(data) - 1:
                usertoken_length = data[password_end]
                usertoken_start = password_end + 1
                usertoken_end = usertoken_start + usertoken_length
                if usertoken_end > len(data) - 1:
                    logging.warning("Usertoken length exceeds data bounds")
                    usertoken = bytearray("NouserToken", "utf-8")
                    return (command, ssid, password, usertoken)
                usertoken = bytearray(data[usertoken_start:usertoken_end])
                return (command, ssid, password, usertoken)
            else:
                logging.warning("Usertoken not found in data")
                usertoken = bytearray("NouserTokenindata", "utf-8")
                return (command, ssid, password,usertoken)
