#!/bin/bash
# Monitor Build 2136 for the imx93-jaguar-eink machine - SECURE BOOT SIGNING TEST
...
echo "🔐 Setting up Build ${BUILD_ID} monitoring - SECURE BOOT SIGNING TEST..."

# Loop to continuously check build status and logs
while true; do
    clear
    echo "🔍 Monitoring Build ${BUILD_ID} - Testing Secure Boot Signing Configuration"
    echo "=================================================================="
    echo "🔐 FIRST BUILD WITH SIGNING ENABLED:"
    echo "   - UBOOT_SIGN_ENABLE=1"
    echo "   - TF_A_SIGN_ENABLE=1" 
    echo "   - OPTEE_TA_SIGN_ENABLE=1"
    echo "=================================================================="
    ...
    if [ "$MAIN_RUN_STATUS" = "FAILED" ]; then
        ...
        echo "🔍 Look for signing-related errors (key generation, signing process, etc.)"
        exit 1
    elif [ "$MAIN_RUN_STATUS" = "PASSED" ]; then
        echo "✅ Main build run PASSED! Build ${BUILD_ID} is complete."
        echo "🔐 SUCCESS: First signed images for imx93-jaguar-eink created!"
        echo "============================================================"
        echo "Build ${BUILD_ID} PASSED. Signed images ready for testing."
        echo "📋 Next steps:"
        echo "   1. Flash signed images to board"
        echo "   2. Verify boot works on unprogrammed device"
        echo "   3. Check for signing artifacts in bootloader"
        exit 0
    else
